...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/12 12:03:19 by jimikim           #+#    #+#             */
/*   Updated: 2021/07/15 12:19:43 by jimikim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_option	*ft_init_opt(char **argv)
{
	...
	if (ft_set_opt_sem(opt) == 0)
	{
		free(opt);
		ft_putstr_fd("ft_set_opt_sem error.\n", 2);
		return (0);
	}
	return (opt);
}
...
t_philo	**ft_init_philo_list(t_option *opt)
{
	...
	if (!philo_list)
	{
		ft_putstr_fd("ft_init_philo_list error.\n", 2);
		free(opt);
		exit(1);
	}
	i = 0;
